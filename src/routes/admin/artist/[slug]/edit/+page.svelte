<script lang="ts">
  ...
  import ArtistForm from "$lib/components/form/artist/ArtistForm.svelte";
  ...
  <section id="images">
    ...
    <UploadImagesForm
      ...
      after_upload={(results) =>
        results.forEach((r) => (r.ok ? artist.images.push(r.data) : void 0))}
    />

    <Separator />

    <div class="flex flex-wrap gap-3">
      {#each artist.images as image (image.id)}
              ...
              <Button
                ...
                onclick={() =>
                  ImageClient.delete(image.id).then((r) => {
                    if (r.ok) {
                      artist.images = Items.remove(artist.images, image.id);
                    }
                  })}
              >
                Delete
      ...
      {:else}
        <Empty
          icon="lucide/frame"
          title="No images"
          description="There haven't been any images added to this artist yet"
        />
      {/each}
    </div>
  </section>
